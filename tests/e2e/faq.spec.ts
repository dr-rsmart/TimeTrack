...
test.describe('FAQ help centre', () => {
  ...
  test('authenticated users can open FAQ topics and compare location permissions', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(
        'Use Allow all the time / Always for background automatic clocking on mobile.',
      ),
    ).toBeVisible();
    ...
    const manualQuestion = page.getByRole('button', {
      name: 'How do I clock in and clock out manually?',
    });
