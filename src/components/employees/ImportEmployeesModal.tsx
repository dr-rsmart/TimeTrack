...
export default function ImportEmployeesModal({ open, onClose, onDone }: ImportEmployeesModalProps) {
  ...
  const handleFileSelected = async (file: File | undefined) => {
    ...
    try {
      ...
      if (parsed.missingRequired.length > 0) {
        toast.error(
          `Missing required column(s): ${parsed.missingRequired.join(', ')}. Download the template to see the expected format.`,
        );
        setPreview(null);
        return;
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.imported > 0) {
        toast.success(
          `Imported ${res.imported} employee(s)${res.skipped > 0 ? ` · ${res.skipped} skipped` : ''}`,
        );
        onDone?.();
      } else {
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start gap-3 rounded-lg border border-brand/30 bg-brand/10 p-3 text-sm">
          ...
          <p className="text-muted-foreground">
            Upload a CSV to onboard up to {MAX_IMPORT_ROWS} employees at once. Each imported
            employee also gets a login account with the temporary password{' '}
                  ...
                  <TableBody>
                    {preview.valid.slice(0, 20).map((r) => (
                      <TableRow key={r.rowNumber} className="hover:bg-transparent">
                        ...
                        <TableCell className="p-2 text-xs">
                          {String(r.payload.email ?? '')}
                        </TableCell>
                        <TableCell className="p-2 text-xs capitalize">
                          {String(r.payload.role ?? 'employee')}
                        </TableCell>
                        <TableCell className="p-2 text-xs">
                          {String(r.payload.branch ?? 'Unassigned')}
                        </TableCell>
                        <TableCell className="p-2 text-xs">
                          {String(r.payload.department ?? 'General')}
                        </TableCell>
                      </TableRow>
                    ))}
