...
| Session revocation         | `User.pwdEpoch` bumped on explicit logout and every password change/reset; JWT claim compared against live value (30s cached, fail-closed); SSE streams closed cluster-wide; self-service rotation re-mints the acting session's cookie with the new epoch so the user stays signed in (login-once-until-logout), while all other pre-rotation tokens stay revoked | `middleware/auth.ts`, `passwords.ts`, `invalidation.ts`, `routes/auth.ts` |
...
- SSE replay buffer (500 events / 5 min) is per-process. Reconnecting to a
  _different_ replica can replay nothing. Sticky sessions (cookie/IP hash)
...
_When you change architecture, update this file in the same PR._
