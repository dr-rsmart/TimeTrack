...
5. **Resubmission (vc21 — remote auto-increment assigned 21; 20 was consumed by an aborted build attempt).** Build with `eas build --platform android --profile production` (remote auto-increment), then roll out with `TT_AAB=timetrack-vc21.aab TT_RELEASE=21 node scripts/update-closed-alpha-vc19.mjs` followed by `TT_AAB=timetrack-vc21.aab TT_RELEASE=21 node scripts/promote-production-vc19.mjs`, replacing the rejected change set. In Play Console, complete the background-location declaration with justification text mirroring the in-app disclosure plus a demo video of the flow in §3.2 item 4, verify the Data-safety location entries (§4.2), then run `node scripts/submit-publishing-vc18.mjs` (generic "Submit N changes for review") and watch with `node scripts/watch-play-review.mjs`.
...
- [x] Native full-screen Prominent Disclosure + consent gate implemented in `mobile/App.js` (vc19), hardened with persisted consent, a single background-start gate and re-disclosure on later opt-in (vc21).
