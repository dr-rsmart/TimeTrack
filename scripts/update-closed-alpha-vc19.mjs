/**
 * Google Play Console — Closed testing ("alpha") rollout updater for VC19
 * ----------------------------------------------------------------------
...
async function attachBundleVc19(page) {
  ...
  if (fromLibrary) {
    await wait(4000);
    const has19Row = await isVisible(page, /App bundle\s+19\s+1\.0\.0/, 4000);
    const picked = has19Row
      ? await clickVisible(
          page,
    ...
    if (!has19Row) {
      console.log(
        'ℹ️  Bundle 19 is not in the library yet — closing the dialog and uploading the .aab instead.',
      );
      ...
      await shot(page, 'library-no-vc19');
    }
    await wait(1500);
    const added = picked
      ? await clickVisible(
...
async function ensureReleaseName(page) {
  try {
    ...
    if ((await nameBox.count()) > 0) {
      ...
      if (!current || !/^\s*19\b/.test(current)) {
...
async function run() {
  ...
  if (hasVc18 && alreadyLive && !hasDraft) {
    console.log('ℹ️ Closed "alpha" already carries Release 19 (1.0.0) — nothing to do.');
    submitted = true;
  } else {
    let inEditor = false;
    if (hasDraft) {
    ...
    if (inEditor) {
      ...
      if (bundleAlready) {
        console.log('ℹ️ vc19 already attached to this release.');
      } else {
  ...
  if (done) {
    console.log('✅ Release 19 (1.0.0) rollout submitted on closed testing "alpha".');
