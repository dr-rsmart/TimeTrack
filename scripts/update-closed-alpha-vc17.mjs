 ...
 * --------------------------------------------------------------
 * Promotes the Closed testing -> alpha track from "Release: 16 (1.0.0)" to
 * "Release: 17 (1.0.0)" by shipping version code 17 (timetrack-vc17 bundle:
 * EAS build #17 — targets API 36 per the Google Play Aug-2026 policy).
 *
 ...
 *   PLAY_CONSOLE_RESULT: DONE    -> release 17 rollout submitted on closed alpha
...
const RELEASE_NAME = '17'; // track row then reads "Release: 17 (1.0.0)"
...
// Attach version code 17 INSIDE the release editor. Preference order:
//  1) pick the already-processed vc17 bundle from the app bundle library,
...
async function attachBundleVc17(page) {
  ...
  if (fromLibrary) {
    await wait(4000);
    // SAFETY: bundle 17 may not exist in the library yet (this is its first
    // Play upload). NEVER click a checkbox blindly — the old .nth(1) fallback
    // once attached bundle 15 under a "17" release name (verified via the
    // before-review screenshot). Only select a row when a "17" version cell
    // is actually visible in the chooser.
    const has17Row = await isVisible(page, /App bundle\s+17\s+1\.0\.0/, 4000);
    const picked = has17Row
      ? await clickVisible(
          ...
          [
            ...
            // unique version-code cell text "17".
            (p) =>
              p.locator(
    ...
    if (!has17Row) {
      console.log(
        'ℹ️  Bundle 17 is not in the library yet — closing the dialog and uploading the .aab instead.',
      );
      ...
      await shot(page, 'library-no-vc17');
    }
    await wait(1500);
    const added = picked
      ? await clickVisible(
...
// bundle (16) into new drafts; a multi-bundle release is the standard Play
// pattern and devices always receive the highest versionCode (17). Removing
...
// toggle (state label "Included"). A release containing both 16 (preloaded)
// and 17 fails validation ("completely shadowed"), and attached-bundle rows
// have NO per-row remove control — so the toggle is the ONLY way to drop 16.
async function excludePreviousRelease(page) {
  const rowVisible = await isVisible(page, /16 \(1\.0\.0\)/, 4000);
...
async function run() {
  ...
  console.log(
    `ℹ️  track state: hasVc17=${hasVc17} hasDraft=${hasDraft} hasLiveRelease16=${await isVisible(page, /16 \(1\.0\.0\)/, 4000)}`,
  );

  let submitted = false;

  if (hasVc17 && alreadyLive && !hasDraft) {
    console.log('ℹ️  Closed "alpha" already carries Release 17 (1.0.0) — nothing to do.');
    submitted = true;
  } else {
    ...
    if (inEditor) {
      ...
      // Play preloads the live release's bundle (16) into new drafts and the
      // 2026 Console offers NO control to remove it; a release containing
      // 16+17 fails validation ("completely shadowed … remove this APK").
      // Recovery: discard this draft, HALT the live release 16 (its review
      // page carries "Halt rollout"), then create a fresh draft that no
      // longer preloads anything.
      const preloaded = await isVisible(page, /16 \(1\.0\.0\)/, 5000);
      if (preloaded) {
        console.log('ℹ️  Live bundle 16 preloaded into the draft — excluding via toggle.');
        ...
        // bundle 16 from this draft (no halt/pause needed).
        const excluded = await excludePreviousRelease(page);
        if (excluded) {
          console.log('✅ Previous release bundle 16 excluded from this draft.');
          await shot(page, 'excluded-16');
        } else {
          console.log('⚠️  Could not exclude previous release bundle 16 — aborting to GUIDED.');
      ...
      // release's bundle (16) into every new draft — that is by design, not
      // dirt. A multi-bundle release is the standard Play pattern and devices
      // always receive the highest versionCode (17). Only a FAILED upload row
      ...
      // Skip bundling if the editor already shows vc17 attached.
  ...
  if (done) {
    console.log('✅ Release 17 (1.0.0) rollout submitted on closed testing "alpha".');
    ...
    console.log('      (or pick Release 17 from the app bundle library)');
