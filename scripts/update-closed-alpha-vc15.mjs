...
async function attachBundleVc15(page) {
  ...
  if (fromLibrary) {
    ...
    const picked = await clickVisible(
      ...
      [
        // Row whose Version code cell is exactly 15 -> tick its checkbox.
        (p) =>
          p
            .locator('tr')
        ...
        (p) =>
          p
            ...
            .filter({ has: p.getByText(/^15$/, { exact: true }) })
...
async function removeStaleBundle(page) {
  ...
  const xbtn = page.locator(
    'xpath=//*[contains(text(),"timetrack-vc14.aab")]/following::button[1]',
  );
  ...
  if (await isVisible(page, /14 \(1\.0\.0\)|timetrack-vc14\.aab/, 2000)) {
    const rm = page.getByRole('button', { name: /remove|delete|dismiss/i }).first();
    if ((await rm.count()) > 0) {
