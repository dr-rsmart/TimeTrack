 ...
 * --------------------------------------------------------------
 * Promotes the Closed testing -> alpha track from "Release: 8 (1.0.0)" to
 * "Release: 9 (1.0.0)" by shipping version code 9 (timetrack-vc9 bundle).
 *
 ...
 *  - Prefers attaching the already-uploaded vc9 bundle from the Play
 *    Console app bundle library; falls back to uploading the .aab.
 *
 * Log markers (for automation watchers):
 *   PLAY_CONSOLE_RESULT: DONE    -> release 9 rollout submitted on closed alpha
...
const AAB_PATH = path.resolve(ROOT, 'timetrack-vc9-no-overlay.aab');
const RELEASE_NAME = '9'; // track row then reads "Release: 9 (1.0.0)"
...
// Attach version code 9 INSIDE the release editor. Preference order:
//  1) pick the already-processed vc9 bundle from the app bundle library,
...
async function attachBundleVc9(page) {
  ...
  if (fromLibrary) {
    ...
    const picked = await clickVisible(
      page,
      'vc9 row checkbox in library chooser',
      [
        // Row whose Version code cell is exactly 8 -> tick its checkbox.
        (p) =>
          p
            .locator('tr')
            .filter({ has: p.locator('td').filter({ hasText: /^9$/ }) })
            .getByRole('checkbox'),
        (p) =>
          p
            .locator('tr')
            .filter({ hasText: /App bundle\s+9\s+1\.0\.0/ })
            .getByRole('checkbox'),
        (p) =>
          p
            .locator('tr')
        ...
        (p) =>
          p
            ...
            .filter({ has: p.getByText(/^9$/, { exact: true }) })
            .locator('input[type="checkbox"], [role="checkbox"], label')
            .first(),
      ],
      20000
    );
    if (!picked) await shot(page, 'library-no-vc9');
    await wait(1500);
    ...
    if (picked && added) {
      console.log('✅ Attached vc9 from the app bundle library.');
      await wait(30000);
      return true;
    }
  }
...
// ── Release name box: ensure it reflects release 9 ──
...
async function run() {
  ...
  const hasVc9 = await isVisible(page, /9 \(1\.0\.0\)/, 6000);
  ...
  console.log(
    `ℹ️  track state: hasVc9=${hasVc9} hasDraft=${hasDraft} hasLiveRelease8=${await isVisible(page, /8 \(1\.0\.0\)/, 4000)}`
  );

  let submitted = false;

  if (hasVc9 && alreadyLive && !hasDraft) {
    console.log('ℹ️  Closed "alpha" already carries Release 9 (1.0.0) — nothing to do.');
    submitted = true;
  } else {
    ...
    if (inEditor) {
      ...
      // Skip bundling if the editor already shows vc9 attached.
      const bundleAlready = await isVisible(page, /9 \(1\.0\.0\)/, 5000);
      if (bundleAlready) {
        console.log('ℹ️  vc9 already attached to this release.');
      } else {
        await attachBundleVc9(page);
      }
  ...
  if (done) {
    console.log('✅ Release 9 (1.0.0) rollout submitted on closed testing "alpha".');
    ...
    console.log('      (or pick Release 9 from the app bundle library)');
