/**
 * Google Play Console — Closed testing ("alpha") rollout updater for VC21
 * ----------------------------------------------------------------------
 * Promotes the Closed testing -> alpha track from "Release: 18 (1.0.0)" to
 * "Release: 21 (1.0.0)" by shipping version code 21 (timetrack-vc21 bundle).
 *
 ...
 *   TT_AAB=timetrack-vc21.aab TT_RELEASE=21 node scripts/update-closed-alpha-vc21.mjs
 *
 * Log markers (for automation watchers):
 *   PLAY_CONSOLE_RESULT: DONE    -> release 21 rollout submitted on closed alpha
...
const AAB_PATH = path.resolve(ROOT, process.env.TT_AAB || 'timetrack-vc21.aab');
const RELEASE_NAME = process.env.TT_RELEASE || '21'; // track row then reads "Release: N (1.0.0)"
...
// Attach version code 21 INSIDE the release editor
async function attachBundleVc21(page) {
  ...
  if (fromLibrary) {
    await wait(4000);
    const has21Row = await isVisible(page, /App bundle\s+21\s+1\.0\.0/, 4000);
    const picked = has21Row
      ? await clickVisible(
          page,
          'vc21 row checkbox in library chooser',
          [
            (p) =>
              p.locator(
                'xpath=(//*[normalize-space(text())="21"]/preceding::*[self::input[@type="checkbox"] or @role="checkbox"])[last()]',
              ),
            (p) =>
              p
                .locator('tr')
                .filter({ has: p.locator('td').filter({ hasText: /^21$/ }) })
                .getByRole('checkbox'),
            (p) =>
              p
                .locator('tr')
                .filter({ hasText: /App bundle\s+21\s+1\.0\.0/ })
                .getByRole('checkbox'),
          ],
          20000,
        )
    ...
    if (!has21Row) {
      console.log(
        'ℹ️  Bundle 21 is not in the library yet — closing the dialog and uploading the .aab instead.',
      );
      ...
      await shot(page, 'library-no-vc21');
    }
    await wait(1500);
    const added = picked
      ? await clickVisible(
      ...
      : false;
    if (picked && added) {
      console.log('✅ Attached vc21 from the app bundle library.');
      await wait(30000);
      return true;
    }
  }
...
async function ensureReleaseName(page) {
  try {
    ...
    if ((await nameBox.count()) > 0) {
      ...
      if (!current || !/^\s*21\b/.test(current)) {
...
async function excludePreviousRelease(page) {
  const rowVisible = await isVisible(page, /18 \(1\.0\.0\)/, 4000);
...
async function run() {
  ...
  const hasVc18 = await isVisible(page, /21 \(1\.0\.0\)/, 6000);
  const hasDraft =
  ...
  console.log(`ℹ️ track state: hasVc18=${hasVc18} hasDraft=${hasDraft}`);

  let submitted = false;

  if (hasVc18 && alreadyLive && !hasDraft) {
    console.log('ℹ️ Closed "alpha" already carries Release 21 (1.0.0) — nothing to do.');
    submitted = true;
  } else {
    let inEditor = false;
    if (hasDraft) {
    ...
    if (inEditor) {
      ...
      const preloaded = await isVisible(page, /18 \(1\.0\.0\)/, 5000);
      if (preloaded) {
        console.log('ℹ️ Live bundle 18 preloaded into the draft — excluding via toggle.');
        const excluded = await excludePreviousRelease(page);
        if (excluded) {
          console.log('✅ Previous release bundle 18 excluded from this draft.');
          await shot(page, 'excluded-18');
        } else {
          console.log('⚠️ Could not exclude previous release bundle 18 — aborting to GUIDED.');
      ...
      if (dirty) {
        console.log('ℹ️ Draft is dirty — discarding it...');
        await page.keyboard.press('Escape').catch(() => {});
        await wait(1000);
      ...
      if (bundleAlready) {
        console.log('ℹ️ vc21 already attached to this release.');
      } else {
  ...
  if (done) {
    console.log('✅ Release 21 (1.0.0) rollout submitted on closed testing "alpha".');
