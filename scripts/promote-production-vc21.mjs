/**
 * Google Play Console — PRODUCTION rollout for release 21 (1.0.0)
 * ---------------------------------------------------------------
 * Ships the vc21 app bundle to the Production track with upbeat release notes.
 *
 ...
 *   TT_AAB=timetrack-vc21.aab TT_RELEASE=21 node scripts/promote-production-vc21.mjs
 *
...
const RELEASE_NAME = process.env.TT_RELEASE || '21'; // production row then reads "Release: N (1.0.0)"
...
async function attachBundleVc21(page) {
  ...
  if (fromLibrary) {
    ...
    if (!picked) await shot(page, 'library-no-vc21');
    await wait(1500);
  ...
  try {
    ...
    console.log('⚠️  Could not attach the vc21 bundle automatically.');
...
async function run() {
  ...
  if (hasVc18 && alreadyLive && !hasDraft) {
    ...
    if (inEditor) {
      ...
      if (bundleAlready) {
        console.log('ℹ️ vc21 already attached — skipping exclude/attach.');
      } else {
  ...
  if (done) {
    console.log('✅ Release 21 (1.0.0) submitted to PRODUCTION (Google review pending).');
