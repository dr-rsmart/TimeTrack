 ...
 * --------------------------------------------------------------
 * Promotes the Closed testing -> alpha track from "Release: 9 (1.0.0)" to
 * "Release: 11 (1.0.0)" by shipping version code 11 (timetrack-vc11 bundle:
 * multi-location geofencing, weekly shift schedules, re-clock guard).
 *
 ...
 *  - Prefers attaching the already-uploaded vc11 bundle from the Play
 *    Console app bundle library; falls back to uploading the .aab.
 *
 * Log markers (for automation watchers):
 *   PLAY_CONSOLE_RESULT: DONE    -> release 11 rollout submitted on closed alpha
...
const AAB_PATH = path.resolve(ROOT, 'timetrack-vc11-multiloc.aab');
const RELEASE_NAME = '11'; // track row then reads "Release: 11 (1.0.0)"
...
// Attach version code 11 INSIDE the release editor. Preference order:
//  1) pick the already-processed vc11 bundle from the app bundle library,
...
async function attachBundleVc11(page) {
  ...
  if (fromLibrary) {
    ...
    const picked = await clickVisible(
      page,
      'vc11 row checkbox in library chooser',
      [
        // Row whose Version code cell is exactly 11 -> tick its checkbox.
        (p) =>
          p
            .locator('tr')
            .filter({ has: p.locator('td').filter({ hasText: /^11$/ }) })
            .getByRole('checkbox'),
        (p) =>
          p
            .locator('tr')
            .filter({ hasText: /App bundle\s+11\s+1\.0\.0/ })
            .getByRole('checkbox'),
        (p) =>
          p
            .locator('tr')
            .filter({ hasText: /Aug 30, 2026/ })
            .getByRole('checkbox'),
        (p) =>
          p
            .locator('tr')
            .filter({ hasText: /App bundle/ })
            .filter({ has: p.getByText(/^11$/, { exact: true }) })
...
// ── Release name box: ensure it reflects release 11 ──
async function ensureReleaseName(page) {
  try {
    ...
    if ((await nameBox.count()) > 0) {
      ...
      if (!current || !/^\s*11\b/.test(current)) {
...
async function run() {
  ...
  if (hasVc11 && alreadyLive && !hasDraft) {
    console.log('ℹ️  Closed "alpha" already carries Release 11 (1.0.0) — nothing to do.');
    submitted = true;
  } else {
    ...
    if (inEditor) {
      ...
      // Skip bundling if the editor already shows vc11 attached.
      const bundleAlready = await isVisible(page, /11 \(1\.0\.0\)/, 5000);
      if (bundleAlready) {
        console.log('ℹ️  vc11 already attached to this release.');
      } else {
        await attachBundleVc11(page);
      }
  ...
  if (done) {
    console.log('✅ Release 11 (1.0.0) rollout submitted on closed testing "alpha".');
    ...
    console.log('      (or pick Release 11 from the app bundle library)');
