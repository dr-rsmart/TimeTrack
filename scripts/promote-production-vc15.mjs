/**
 * Google Play Console — PRODUCTION rollout for release 15 (1.0.0)
 * ---------------------------------------------------------------
 * Ships the vc15 app bundle (EAS build #15, targets API 36 per the Google
 * Play Aug-2026 policy — already live on Closed testing "alpha") to the
 * Production track with upbeat release notes.
 *
 * Conventions match scripts/update-closed-alpha-vc15.mjs:
...
const RELEASE_NOTES = [
  '🎉 TimeTrack 1.0.0 — our biggest launch yet!',
  '⚡ Blazing-fast, buttery-smooth performance on every screen',
  '📍 Effortless hands-free attendance with smart geofence auto clock-in/out',
  '📊 Beautiful real-time dashboards your whole team will love',
  '🔒 Rock-solid security and privacy you can trust',
  '🛠 Tons of stability improvements and polish inspired by your feedback',
  'Thank you for being part of the journey — this one is for you! 💙',
].join('\n');
...
// Attach version code 15 INSIDE the release editor (library first, then upload).
async function attachBundleVc15(page) {
  ...
  try {
    ...
    console.log('⚠️  Could not attach the vc15 bundle automatically.');
...
async function run() {
  ...
  console.log(
    `ℹ️  production state: hasVc15=${hasVc15} alreadyLive=${alreadyLive} hasDraft=${hasDraft}`,
  );

  let submitted = false;
  if (hasVc15 && alreadyLive && !hasDraft) {
    console.log('ℹ️  Production already carries release 15 (1.0.0) — nothing to do.');
    submitted = true;
  } else {
    let inEditor = false;
    if (hasDraft) {
  ...
  if (done) {
    console.log('✅ Release 15 (1.0.0) submitted to PRODUCTION (Google review pending).');
    ...
    console.log('   2. Attach vc15 from the app bundle library (or upload');
