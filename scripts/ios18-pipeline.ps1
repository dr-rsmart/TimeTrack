...
$buildId = 'f2d91d51-6c59-4a45-a65d-e3b343af3b82'
...
while ((Get-Date) -lt $deadline) {
  $raw = Get-Content "$env:TEMP\eas-build-ios18.log" -Raw -ErrorAction SilentlyContinue
...
if (-not $artifact -and -not ((Get-Content "$env:TEMP\eas-build-ios18.log" -Raw) -match 'Build finished')) {
...
cmd /c "npx eas-cli submit --platform ios --latest --non-interactive > `"$env:TEMP\eas-ios-submit18.log`" 2>&1"
$sub = Get-Content "$env:TEMP\eas-ios-submit18.log" -Raw
if ($sub -notmatch 'Submitted your app') { Log 'EAS submit did not report success — check eas-ios-submit18.log. Aborting.'; exit 1 }
Log 'EAS reports binary uploaded.'

# ── Step 3: verify build 18 actually lands in ASC and becomes VALID ──
Log 'Step 3: waiting for ASC to show build 18 VALID (max 45 min)...'
...
const t=p+'.'+b64(sig);
(async()=>{
  const r=await fetch('https://api.appstoreconnect.apple.com/v1/builds?filter[app]=6803827296&filter[version]=18&limit=2',{headers:{Authorization:'Bearer '+t}});
...
"@
  Log "ASC build 18 state: $out"
  if ($out -match 'VALID') { $valid = $true; break }
  if ($out -match 'INVALID|FAILED') { Log 'Apple marked build 18 INVALID — check the Invalid Binary email. Aborting.'; exit 1 }
  Start-Sleep 45
}
if (-not $valid) { Log 'Build 18 never appeared in ASC (silent drop again). Aborting — do NOT submit build 11.'; exit 1 }

# ── Step 4: attach + submit for review ──
Log 'Step 4: attaching build 18 and submitting for App Review...'
