/**
 * Google Play Console — finish the vc8 closed-alpha rollout
 * ----------------------------------------------------------
 * The vc8 draft release already exists on Closed testing -> alpha with the
 * processed bundle attached, but a duplicate-upload error ("version code 8
 * has already been used") left the editor with a disabled "Next" button.
 *
 * This focused script:
 *  1) opens the draft editor,
 *  2) discards the unsaved duplicate-upload error state (restores the saved
 *     draft carrying the good vc8 bundle),
 *  3) re-attaches vc8 from the app bundle library ONLY if the bundle row is
 *     actually missing,
 *  4) completes Next -> Start rollout to Closed testing -> Confirm using
 *     actionability-aware clicks (waits for buttons to become enabled).
 *
 * Log markers: FINISH8: DONE | FINISH8: GUIDED | FINISH8: ERROR
 */
...
async function shot(page, name) {
  ...
  try {
    await page.screenshot({ path: path.resolve(ROOT, `play-console-finish8-${name}.png`) });
    console.log(`📸 saved play-console-finish8-${name}.png`);
  } catch {}
}
...
async function run() {
  ...
  // ── 2) Ensure the vc8 bundle row is present ──
  let hasBundle = await isVisible(page, /8 \(1\.0\.0\)/, 6000);
  if (!hasBundle) {
    hasBundle = await isVisible(page, /timetrack-vc8-webview-failsafe\.aab/, 4000);
  }
  if (!hasBundle) {
    ...
    console.log('✅ vc8 bundle row present in the draft.');
  }

  // ── 3) Next (actionability-aware: waits until enabled) ──
  const next = page.getByRole('button', { name: /^next$/i }).first();
  await next.click({ timeout: 90000 }).catch(async (e) => {
  ...
  if (done) {
    ...
    console.log('FINISH8: DONE');
  } else {
    console.log('FINISH8: GUIDED — complete the rollout in the open browser window.');
